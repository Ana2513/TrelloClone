...
class cList {
  ...
  changeTitle() {
    this.h1.style.visibility = 'hidden'
    this.bntAddCard.style.visibility = 'hidden'
    this.inputTitle = document.createElement('input')
    this.inputTitle.classList.add('inputTitle')
    this.inputTitle.value = this.title
    this.divHeader.prepend(this.inputTitle)

    this.inputTitle.addEventListener('keyup', (e) => {
      if (e.key === 'Enter' && this.inputTitle.value != '') {
        e.preventDefault()

        let textTitle = this.inputTitle.value

        this.title = textTitle
        this.h1.innerText = textTitle
        this.h1.style.visibility = 'visible'
        this.bntAddCard.style.visibility = 'visible'
  ...
  createListElement() {
    ...
    this.listCard.addEventListener('dragover', (e) => {
      ...
      let cardAfterDraggingCard = getCardAfterDraggingCard(
        this.listCard,
        e.clientY
      )
...
addList.addEventListener('click', () => {
  let list = new cList(board, 'New List')
})
